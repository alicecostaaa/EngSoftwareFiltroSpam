...
{	
	...
	public boolean classify(String msg)
	{
		...
		spamSum += Math.log(spam_probability);
                ...
                //System.out.println(hamSum+" "+spamSum);
		if(hamSum >= spamSum) 
                    return false;
	...
	//---------------TRAINING METHODS USING AS FEATURES ALL THE WORDS THAT WERE FOUND IN MESSAGES---------------//
        ...
        //Variaveis vão receber op caminho em interfaces.TelaConjunta e serem setadas aqui
	private String ling;
        private String spam;
    ...
    public boolean isTaSendoExecutado() {
        return taSendoExecutado;
    }

    //public void train(String lingFilesPath , String spamFilesPath)
    public void setTaSendoExecutado(boolean taSendoExecutado) {
        this.taSendoExecutado = taSendoExecutado;
    }
    boolean taSendoExecutado = false;  
    public void train()
	{
		trainLing();
		trainSpam();
	...
	//private void trainLing(String lingFilesPath)
      private void trainLing()
	{
		try
		{
			//File directory = new File(lingFilesPath);
                    File directory = new File(ling);
